...
Adds three paper-grade analyses on top of existing CasFly artifacts:

  (1) Downstream predictive utility (LaVE vs PCMCI+): for each device and
      each tau, trains one logistic-regression model per effect_bin using
      lagged cause features, concatenates predictions across effects, and
      computes AUROC / AUPRC / LogLoss / Brier score.

  (2) Reverse Hits@K: checks whether each PCMCI+ edge appears in LaVE's
      top-K predictions for the same (device, cause_bin).

  (3) Paired statistics across devices: Wilcoxon signed-rank test and
      Cliff's delta comparing LaVE vs PCMCI+ AUROC/AUPRC per tau.

Inputs:
    devices_root/Device*/Patient_*.csv  -- observation CSVs with columns
                                           DESCRIPTION, VALUE, DATE
    lave_csv                            -- lave_edges_normalized.csv
                                           (device, cause_bin, effect_bin, lag_bin)
    pcmci_root_base                     -- path prefix; tau is appended, e.g.
                                           artifacts/.../pcmci_tau_
                                           Each tau dir contains Device*/edges.csv

Outputs (under out_root):
    predictive/per_device_metrics.csv
    predictive/summary_by_tau.csv
    predictive/paired_tests.csv
    reverse_hits/reverse_hits_by_tau.csv
"""
...
# Clinically motivated value bins (must match compare_pcmci_lave.py)
...
def lag_center(lag_bin: str) -> Optional[int]:
    """Convert '7-14' to integer midpoint 10 for use as a shift offset."""
...
def build_daily_matrix(device_dir: Path, min_days_present: int=1) -> pd.DataFrame:
    """Aggregate per-patient CSVs into a daily binary event-bin matrix."""
    rows = []
...
def load_pcmci_edges_for_tau(pcmci_root: Path) -> pd.DataFrame:
    """Load all Device*/edges.csv files under a single tau's PCMCI output dir."""
    rows = []
...
def reverse_hits_at_k(lave: pd.DataFrame, pcmci: pd.DataFrame, ks=(1,3,5)) -> pd.DataFrame:
    """Check how often each PCMCI+ edge appears in LaVE's top-K for the same cause."""
...
def columns_for_bin(daily: pd.DataFrame, bin_str: str) -> List[str]:
    """Find all daily matrix columns whose bin suffix matches bin_str."""
    suffix = f"[{bin_str}]"
...
def agg_bin_series(daily: pd.DataFrame, bin_str: str) -> Optional[pd.Series]:
    """OR-aggregate all columns sharing a bin into a single binary series."""
    cols = columns_for_bin(daily, bin_str)
    if not cols:
        return None
    s = daily[cols].max(axis=1)
...
def build_Xy_for_effect(daily: pd.DataFrame,
    ...
    """
    Build (X, y) for a single effect_bin.
    Each cause column is shifted by its lag midpoint to form a feature.
    Returns None if the dataset is degenerate.
    """
    ...
    # Drop the first max_lag rows which contain NaNs from shifting
    X  = X[max_lag:]
    yv = yv[max_lag:]
...
def evaluate_edges_predictive(daily: pd.DataFrame,
    ...
    """
    Train one logistic model per effect_bin, collect predictions across
    effects, and compute global AUROC/AUPRC/LogLoss/Brier.
    Uses a chronological 70/30 split where possible; falls back to
    stratified random split for very short series.
    """
    if daily.empty or edges.empty:
        return dict(auroc=np.nan, auprc=np.nan, logloss=np.nan, brier=np.nan, n=0)

    # Accumulated true labels and predicted probabilities across all per-effect models
    all_true_labels = []
    all_pred_probs  = []

    for effect_bin, sub in edges.groupby("effect_bin"):
        dataset = build_Xy_for_effect(daily, effect_bin, sub)
        if dataset is None:
            continue
        X, y = dataset
        n = X.shape[0]
        ...
        if split <= 1 or split >= n-1:
            ...
            # Time-aware split: train on earlier data, test on later
        ...
        all_true_labels.append(yte)
        all_pred_probs.append(pr)

    if not all_true_labels:
        return dict(auroc=np.nan, auprc=np.nan, logloss=np.nan, brier=np.nan, n=0)

    # Concatenate across all effects for a single device-level score
    y_cat = np.concatenate(all_true_labels)
    p_cat = np.concatenate(all_pred_probs)
...
def cliffs_delta(x: np.ndarray, y: np.ndarray) -> float:
    """Cliff's delta (x vs y): in [-1,1]; >0 means x tends to be larger."""
...
def main():
    ...
    ap.add_argument("--pcmci_root_base", required=True,
                    help="prefix like artifacts/.../pcmci_tau_ (tau value appended)")
    ...
    # Pre-build daily matrices once per device (reused across all tau values)
    ...
    for tau in taus:
        ...
        for metric in ["auroc","auprc"]:
            ...
            try:
                # One-sided test: LaVE > PCMCI+
                stat, p = wilcoxon(x, y, alternative="greater")
            except Exception:
                stat, p = np.nan, np.nan
            delta = cliffs_delta(x, y)  # positive means LaVE is larger
            paired_out.append(dict(tau=tau, metric=metric, N=len(x),
                                   wilcoxon_stat=stat, p_value=p, cliffs_delta=delta))
    ...
    print("\n=== Predictive utility (macro by tau) ===")
