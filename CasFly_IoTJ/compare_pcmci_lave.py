...
PCMCI+ vs LaVE -- tau sweep with tables and plots (resume-aware).

Pipeline overview:
  1. For each tau value, trains PCMCI+ per device on daily event-bin matrices
     built from per-patient CSVs.  Skips devices already processed unless
     --force_pcmci is set.
  2. Extracts LaVE causal edges once from allmerged_metrics_log.csv (the
     'Chain Data' column) and normalises them to (device, cause_bin,
     effect_bin, lag_bin) tuples.  Skips if the output CSV already exists
     unless --force_lave is set.
  3. Computes edge overlap, precision/recall/F1, and Hits@K for each tau.
     Cached per-tau CSVs are reused unless --force_eval is set.
  4. Writes consolidated tables and paper-ready plots under out_root.

Inputs:
    devices_root/Device*/Patient_*.csv  -- per-patient observation CSVs with
                                           columns DESCRIPTION, VALUE, DATE
    log_csv                             -- merged LaVE run log with a
                                           'Chain Data' column

Outputs (all under out_root):
    lave_edges_normalized.csv
    pcmci_tau_{tau}/Device*/edges.csv
    tau_{tau}/edge_overlap_counts.csv
    tau_{tau}/computed_metrics_from_overlap.csv
    tau_{tau}/hits_at_k.csv
    tables/macro_by_tau.csv
    tables/per_device_metrics_by_tau*.csv
    tables/overlap_by_tau.csv
    tables/hits_at_k_by_tau.csv
    plots/*.png

Example:
    python3 compare_pcmci_lave.py \\
      --devices_root data/synthea/device_scripts \\
      --log_csv allmerged_metrics_log.csv \\
...
# Reusable hatch sequence for B/W friendly bars
_HATCH_SEQ = ['///', '\\\\\\', 'xxx', '---', '+++', '...', 'oo', '***', '**', '///***']
...
# Clinically motivated value bins aligned with LaVE's own binning
...
def bin_value(event: str, value) -> Optional[str]:
    """Map (event_name, raw_value) to a bin label like 'Glucose[70-99]'."""
...
def build_daily_matrix(device_dir: Path, min_days_present: int) -> pd.DataFrame:
    """Aggregate all Patient_*.csv files in device_dir into a daily binary event matrix."""
    rows = []
...
def run_pcmci_on_matrix(df: pd.DataFrame, tau_max: int, alpha: float):
    """Run PCMCI+ and return significant directed edges as a list of tuples."""
    ...
    try:
        ...
        return []
    val_matrix = res.get("val_matrix")
    p_matrix   = res.get("p_matrix")
    if val_matrix is None or p_matrix is None:
        return []
    edges = []
    N = len(var_names)
    for i in range(N):
        for j in range(N):
            ...
            for tau in range(1, tau_max + 1):
                val_ij = val_matrix[i, j, tau]
                p_ij   = p_matrix[i, j, tau]
                if np.isfinite(val_ij) and np.isfinite(p_ij) and p_ij <= alpha:
                    edges.append((var_names[i], var_names[j], tau, float(val_ij), float(p_ij)))
    edges.sort(key=lambda x: abs(x[3]), reverse=True)
    return edges

def split_evt_bin(lbl: str) -> Tuple[str, Optional[str]]:
    """Split 'Glucose[70-99]' into ('Glucose', '70-99')."""
...
def train_pcmci_per_device(devices_root: Path, out_root: Path,
    ...
    for dev_dir in dev_dirs:
        ...
        if not matrix_is_usable(mat, tau_eff):
            print(f"[PCMCI] {dev}: matrix too sparse/short for tau={tau_max} (T={T}, N={mat.shape[1]}). Writing empty edges.csv")
            save_pcmci_edges(dev, [], dev_out); continue
        print(f"[PCMCI] {dev}: T={T} days, N={mat.shape[1]} events -> running PCMCI+ (tau_eff={tau_eff}, alpha={alpha}) ...")
...
def parse_chain(chain_str) -> Iterable[dict]:
    """Parse a Chain Data string into a list of edge dicts."""
    ...
    out = []
    for step in chain:
        # Each step is [cause_label, effect_label, prob, lag, 'DeviceX']
        ...
        # Only keep steps where both labels are pure bin ranges (e.g. '70-99')
...
def overlap_counts(lave: pd.DataFrame, pcmci: pd.DataFrame) -> pd.DataFrame:
    """Compute per-device edge overlap between LaVE and PCMCI+ edge sets."""
    stats = []
    devices = sorted(set(lave["device"]).union(pcmci["device"]))
    for dev in devices:
        lave_edge_set  = set(map(tuple, lave[lave["device"]==dev][["cause_bin","effect_bin"]].values))
        pcmci_edge_set = set(map(tuple, pcmci[pcmci["device"]==dev][["cause_bin","effect_bin"]].values))
        both      = len(lave_edge_set & pcmci_edge_set)
        lave_only = len(lave_edge_set - pcmci_edge_set)
        pcmci_only = len(pcmci_edge_set - lave_edge_set)
        stats.append(dict(device=dev, lave_only=lave_only, both=both, pcmci_only=pcmci_only,
                          pcmci_edges=len(pcmci_edge_set), lave_edges=len(lave_edge_set)))
    return pd.DataFrame(stats)

def metrics_from_counts(df_counts: pd.DataFrame) -> pd.DataFrame:
    """Compute precision, recall, F1 from overlap counts (treating LaVE as ground truth)."""
    rows = []
...
def hits_at_k(lave: pd.DataFrame, pcmci: pd.DataFrame, ks=(1,3,5)) -> pd.DataFrame:
    """Compute Hits@K: how often the true LaVE effect is in PCMCI's top-K predictions."""
...
def plot_macro_metrics_vs_tau(macro_df: pd.DataFrame, per_dev_df: pd.DataFrame, out_dir: Path):
    """Mean +/- 95% CI across devices for each tau."""
    g = (per_dev_df
    ...
    ax.set_ylim(0, 1.0)
    _apply_axes_style(ax, x_label="tau (days)", y_label="Score")
...
def _heatmap(matrix: pd.DataFrame, out_path: Path, y_label="Device", x_label="tau (days)", vmin=0.0, vmax=1.0):
...
def plot_overlap_stacked_bars(overlap_by_tau: pd.DataFrame, out_dir: Path):
    ...
    _apply_axes_style(ax, x_label="tau (days)", y_label="# edges")
    _legend(ax, loc='upper left', ncol=1)
...
def plot_f1_boxplot(per_dev_df: pd.DataFrame, taus: List[int], out_dir: Path):
    ...
    # Collect per-tau F1 arrays for the boxplot
    ...
    _apply_axes_style(ax, x_label="tau (days)", y_label="F1")
    ax.set_ylim(0, 1.0)
    fig.tight_layout()
...
def plot_complexity_scatter(per_dev_df: pd.DataFrame, out_dir: Path):
    ...
    for i, t in enumerate(taus):
        dft = df[df["tau"]==t]
        ax.scatter(dft["pcmci_edges"], dft["f1"], label=f"tau={t}", color=_color(i), edgecolor='black', linewidth=0.6)
...
def plot_hits_at_k_by_tau(hits_df: pd.DataFrame, out_dir: Path):
    ...
    _apply_axes_style(ax, x_label="tau (days)", y_label="Hits")
...
def run_for_tau(devices_root: Path, log_csv: Path, base_out: Path,
    ...
    if pcmci.empty:
        print("[WARN] PCMCI produced no edges for this tau.")

    if resume and cached_tau_outputs_exist(tau_dir) and not force_eval:
        print(f"[SKIP] Metrics already computed for tau={tau}. Using cached CSVs.")
        ...
        print(f"Macro-avg (tau={tau}): {macro}")
    ...
    # Quick per-tau bar chart saved in the tau's plots subfolder
...
def write_tables(out_root: Path,
    ...
    with open(tbl_dir / "README_tables.md", "w") as fh:
        ...
        fh.write("- macro_by_tau.csv: macro averages (precision, recall, F1) across devices for each tau.\n")
        fh.write("- per_device_metrics_by_tau.csv: per-device metrics per tau (long format).\n")
        fh.write("- per_device_metrics_by_tau_wide_*.csv: device x tau matrices for F1/precision/recall.\n")
        fh.write("- overlap_by_tau.csv: sum of overlaps across devices per tau.\n")
        fh.write("- hits_at_k_by_tau.csv: hits and hit_rate per tau and K.\n")
...
def make_paper_plots(out_root: Path,
    ...
    plots_dir.mkdir(parents=True, exist_ok=True)
    plot_macro_metrics_vs_tau(macro_df, per_dev_df, plots_dir)
...
def main():
    ap = argparse.ArgumentParser(description="tau sweep with tables & plots (PCMCI+ vs LaVE) -- resume-aware")
    ...
    ap.add_argument("--force_eval", action="store_true", help="recompute per-tau metrics even if cached")
